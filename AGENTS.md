...
- `cd backend && yarn lint` – ESLint across `src` and `test` without mutating files.
- `cd backend && yarn lint:fix` / `yarn format` – auto-fix ESLint or Prettier issues intentionally.
...
- `cd frontend && yarn lint` / `yarn lint:fix` – ESLint check or auto-fix.
...
- Generated frontend files under `frontend/src/api/**` and `frontend/src/routeTree.gen.ts` must not be hand-edited. Regenerate them with `yarn orval` or the TanStack router build/plugin, then format generated output.
- Backend Jest mocks may use relaxed unsafe-`any` rules in `test/**/*.ts`; keep those exceptions contained to tests and do not mirror them in `src`.
