...
function validateReportProvenance(
  ...
  if (report.provenance) {
    if (
      ...
      !approval.approvedRunnerHashes.includes(report.harness?.files?.['benchmark-runner.cjs'])
    )
      throw new Error(
...
function approveProvenance(args) {
  ...
  const approval = {
    ...
    approvedRunnerHashes: [hash(fs.readFileSync(path.join(__dirname, 'benchmark-runner.cjs')))],
    variants: {
      before: {
