...
async function compare(args) {
  ...
  const selectedRows = arg(args, 'rows', '').split(',').filter(Boolean).map(Number);
  ...
  const reports = fs
    ...
    ).filter(report => !selectedRows.length || selectedRows.includes(report.fixture.rows));
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ exploratory, rowScope: selectedRows.length ? selectedRows : 'all available rows in the selected suite', comparisons, summary }, null, 2) + '\n',
  );
  const table = [
