...
For observed net-worth discrepancies, get_balance_history returns chart values and opening/closing endpoint evidence. get_balance_change_attribution offers the same exact signed changes ranked by account, including credit/loan liabilities. Effective balance is current balance for every account type, not available balance. A credit/loan contribution is negative debt even when its stored balance sign is positive. Changes are recorded endpoint differences, not market P&L or spending causation. Exact reconciliation can coexist with an unsupported explanation.

Each historical endpoint selects the latest balance snapshot on or before the requested date; a prior snapshot is carried forward, never a future balance snapshot. Before an account's first snapshot the chart contributes zero and coverage=missing, not a known zero valuation. The current owned account set and classifications are used, including archived accounts. snapshotId/date/type and snapshotUpdatedAt identify the selected recording; recording update time is not a market price time. Stored FORWARD_FILL snapshots are also labeled carried forward; their original source recording is not stored. latestAccountSyncAt (and legacy summary syncedAt) can be later than the historical date and cannot explain a past chart point.

Native amounts are preserved. reportingBalance/reportingEffectiveBalance and raw snapshot reportingCurrentBalance/reportingAvailableBalance are explicit even for matching currencies. IDENTITY has rate 1. Nonzero history conversion missing FX fails the tool; raw snapshots/current account reads preserve native data with null reporting balance when FX is unavailable. Zero balances can be reported in the target currency without a quote. FX is applied at each requested chart date, not at the carried snapshot date; actual rateDate and source distinguish DB exact, FORWARD_FILLED prior, and BACKWARD_FILLED later-rate fallback. No upstream vendor provenance is stored beyond this DB evidence. get_accounts_snapshot reads current account records with explicitly labeled UTC current-date FX, separate from historical snapshots. Raw list_balance_snapshots contains only recorded rows, not every carried-forward chart date.

Use list_holdings_snapshot_dates to discover factual investment holdings coverage in a bounded range; narrow or partition when truncated. list_investment_holdings keeps exact-date selection as the default for snapshotDate. dateMode=on_or_before requires minSnapshotDate and selects only past headers within those bounds; no date means latest. Missing snapshotDate=null differs from a recorded date with holdingCount=0 (empty). Holdings are separate from balance snapshots and sparse dates do not prove unchanged quantities. Institution price-as-of timestamps are stored with positions. Security identifiers, exchange/currency and security close-price metadata are current security metadata, not guaranteed historical metadata.

get_historical_valuation_evidence reads up to 20 owned securities and 366 inclusive days. It reuses Yahoo historical daily prices only for stored Yahoo identities; other securities expose stored institution prices and explicit unmapped status, without guessing an ISIN/ticker proxy. Price endpoints select on or before within the evidence range; an unavailable opening quote needs a broader bounded evidence read and explicit client date selection. Prices use exchange-local dates for Yahoo and institutionPriceAsOf otherwise. Returned FX includes quote-date and valuation-date evidence with actual fallback dates. Provider empty, error and unavailable status differ; ambiguous stored same-date prices and stored-price truncation are explicit. Do not silently mix institution prices, close, and adjustedClose bases. Provider adjustment and corporate-action completeness are unverified. A client can calculate a constant-holdings estimate only with explicit unchanged-quantity, price-basis, currency and date-alignment assumptions; splits, dividends, trading, transfers, fees and cash can invalidate it. Keep that estimate separate from observed chart values. Evidence permits investigation of competing explanations and does not prescribe a conclusion.
...
For user-authorized direct transaction corrections, use set_transaction_categories with explicit transaction IDs grouped by target category ID. It supports one or many provider/manual banking transactions, different categories in one atomic batch, and null for explicit Uncategorized. It overwrites existing assignments as manual; no preview token is required. Use list_transactions and list_categories to resolve IDs first. Use categorization-rule tools instead when the user wants future automation.
