...
const outputRoot = path.resolve(
  __dirname,
  '../../docs/performance/backend-query',
);
  ...
  await new Promise((resolve, reject) => {
    ...
    processChild.once('exit', (code, signal) =>
      code === 0
        ? resolve()
        : reject(
            new Error(`Focused ${variant} capture failed: ${code ?? signal}`),
          ),
    );
  });
}
...
async function capture(variant, suite, rows, flags, expected, memory = false) {
  ...
  if (fs.existsSync(file)) {
    ...
    if (
      ...
      existing.samples === 3 &&
      existing.warmups === 1 &&
      existing.scenarios.length === expected
    ) {
      provenance.validateReportProvenance(existing, family);
      console.log(
        `Reusing completed approved focused capture ${suite}/${variant}/${rows}`,
      );
      return;
    }
  }
  await stage(`${suite}/${variant}/${rows}`, () =>
    child(
      variant,
      [
        'capture',
  ...
  if (!report.completed || report.scenarios.length !== expected)
    throw new Error(
      `Incomplete focused shape coverage: ${suite}/${variant}/${rows}`,
    );
}
async function run() {
  for (const variant of ['before', 'after']) {
    if (!roots[variant])
      throw new Error('Set both approved frozen source roots explicitly');
    provenance.validateSourceSelection(path.resolve(roots[variant]), variant);
  }
  save();
  ...
  for (const [suite, rows, flags, expected] of [
    ...
    [
      'filters',
      100000,
      ['--filters', '--filter', 'transactions.filter.', '--no-transports'],
      3,
    ],
    [
      'auth-settings',
    ...
    [
      'extended',
      1000000,
      ['--extended', '--filter', 'extended.', '--no-transports'],
      24,
    ],
    ['sync', 10000, ['--sync', '--filter', 'sync.', '--no-transports'], 17],
  ]) {
    for (const variant of ['before', 'after'])
      await capture(variant, suite, rows, flags, expected);
  }
  for (const variant of ['before', 'after'])
    await stage(`mixed/${variant}`, () =>
      child(variant, [
        'mixed',
        '--variant',
        variant,
  ...
  await stage('comparison', () =>
    require('./focused-comparison.cjs').compareFocused(outputRoot),
  );
  progress.completed = true;
  save();
  console.log(
    'User-approved focused benchmark set completed and all strict source/financial/query-budget gates passed.',
  );
}
module.exports = { run };
